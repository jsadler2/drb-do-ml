...
rule make_predictions:
    input:
        ...
        "../../../out/well_obs_io_ADJ_RDX_IMP_SLP.zarr",
    output:
        "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/preds_adj_RDX_IMP_SLP.feather",
    run:
        weight_dir = input[1] + "/"
...
rule make_filtered_predictions:
    input:
        "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/preds_adj_RDX_IMP_SLP.feather"
    output:
        "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/{partition}_preds_adj_RDX_IMP_SLP.feather"
    run:
...
rule combine_metrics:
     input:
          "../../../out/well_obs_io.zarr",
          "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/trn_preds_adj_RDX_IMP_SLP.feather",
          "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/val_preds_adj_RDX_IMP_SLP.feather",
          "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/val_times_preds_adj_RDX_IMP_SLP.feather"
     output:
          "{outdir}/nstates_{nstates}/nep_{epochs}/rep_{rep}/{metric_type}_metrics_adj_RDX_IMP_SLP.csv"
     params:
         grp_arg = get_grp_arg
     run:
